...
first Empty = error "cant get first"
first (Node a tq) = a
...
dequeue Empty = error "cant get first"
dequeue (Node a tq) = tq
...
enqueue a _ Empty = Node a Empty
enqueue a n (Node b tq)
    | mod a n == mod b n = Node b (Node a tq)
    | otherwise = Node b (enqueue a n tq)
...
mkTQ _ [] = Empty
mkTQ n xs = foldr (\q x -> enqueue q n x) Empty (reverse xs)
...
show' q = "LinearTQueue(" ++ intercalate "," (aux q) ++ ")"
    where
        aux Empty = []
        aux (Node x q) = Prelude.show x : aux q
-- Team Queue equality
